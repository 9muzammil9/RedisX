type MessageHandler = (channel: string, message: string, timestamp: string) => void;
...
export class WebSocketClient {
  ...
  private subscribedChannels = new Map<string, Set<string>>(); // connectionId -> Set<channel>
    ...
    try {
      ...
      this.ws.onopen = () => {
        ...
        if (this.subscribedChannels.size > 0) {
          ...
          for (const [connectionId, channels] of this.subscribedChannels.entries()) {
            if (channels.size > 0) {
              console.log(`🚀 Resubscribing to ${channels.size} channels for ${connectionId}`);
              this.send({
                type: 'subscribe',
                connectionId,
    ...
    switch (response.type) {
      case 'message':
        if (response.channel && response.message && response.data?.timestamp) {
          console.log(`🔔 Processing message: ${response.channel} -> ${response.message}`);
          this.messageHandlers.forEach(handler => 
            handler(response.channel!, response.message!, response.data!.timestamp)
          );
        } else {
  ...
  subscribe(connectionId: string, channels: string[]): void {
    console.log('🚀 Subscribing to channels:', channels, 'for connection:', connectionId);
