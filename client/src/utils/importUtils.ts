 ...
 */
export const detectFileFormat = (content: string): 'json' | 'csv' | 'redis-cli' | 'unknown' => {
  ...
  if (lines.length > 0) {
    ...
    if (firstLine.includes('key') && firstLine.includes('type') && firstLine.includes('value')) {
      return 'csv';
    }
...
export const parseJsonImport = (content: string): ParsedImportData => {
  ...
  try {
    ...
    result.errors.push({
      message: 'Invalid JSON format. Expected object with "keys" array or direct array of key objects.'
    });
...
export const parseCsvImport = (content: string): ParsedImportData => {
  ...
  // Parse header
  const header = lines[0].split(',').map(h => h.trim().replace(/^"(.*)"$/, '$1').toLowerCase());
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    try {
      ...
      try {
        if (type === 'hash' || type === 'list' || type === 'set' || type === 'zset') {
...
export const parseRedisCliImport = (content: string): ParsedImportData => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    try {
      ...
      result.errors.push({
        line: i + 1,
        message: `Error parsing command: ${error instanceof Error ? error.message : 'Unknown error'}`
      });
    }
  }
 ...
 */
export const validateImportData = (data: ImportKeyData[]): Array<{ index: number; message: string }> => {
  ...
  data.forEach((item, index) => {
    ...
    if (!['string', 'list', 'set', 'hash', 'zset'].includes(item.type)) {
      errors.push({ index, message: 'Invalid type. Must be one of: string, list, set, hash, zset' });
    }
      ...
      switch (item.type) {
        ...
        case 'set':
          if (!Array.isArray(item.value)) {
            errors.push({ index, message: `Value for ${item.type} must be an array` });
          }
          break;
        case 'hash':
          ...
          break;
        case 'zset':
          if (!Array.isArray(item.value) || !item.value.every(v => 
            typeof v === 'object' && 'score' in v && 'member' in v
          )) {
            errors.push({ index, message: 'Value for zset must be an array of {score, member} objects' });
          }
          break;
      }
    }
 ...
 */
export const parseImportFile = (content: string, format?: string): ParsedImportData => {
