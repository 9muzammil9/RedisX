...
export async function checkAndRecoverConnection(connection: RedisConnection): Promise<RedisConnection> {
  try {
    ...
    if (response.data.exists) {
      ...
      console.log(`✅ Restored connection with original ID: ${restoredConnection.id}`);
      return restoredConnection;
    }
  } catch (error) {
    console.error(`❌ Failed to check/recover connection ${connection.id}:`, error);
    throw error;
  }
}

export async function recoverAllConnections(connections: RedisConnection[]): Promise<{
