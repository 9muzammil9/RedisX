...
export function saveSubscriptions(connectionId: string, channels: Map<string, boolean>): void {
  try {
    ...
    if (channels.size === 0) {
      ...
      const channelData: ChannelSubscription[] = Array.from(channels.entries()).map(([channel, persistMessages]) => ({
        channel,
        persistMessages
      }));
    ...
    console.log(`💾 Saved subscriptions for ${connectionId}:`, Array.from(channels.keys()));
  } catch (error) {
...
export function saveSubscriptionsLegacy(connectionId: string, channels: Set<string>): void {
    ...
    if (subscription) {
      ...
      if (!isStale) {
        ...
        if (Array.isArray(subscription.channels)) {
          if (subscription.channels.length > 0 && typeof subscription.channels[0] === 'string') {
            // Legacy format: array of strings
            (subscription.channels as string[]).forEach(channel => channelMap.set(channel, false));
          } else {
        ...
        console.log(`📥 Loaded subscriptions for ${connectionId}:`, Array.from(channelMap.keys()));
        return channelMap;
      } else {
        // Remove stale data
